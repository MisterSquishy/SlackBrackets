...
const handle = async ({ message, say }) => {
  try {
    ...
    const results = matches.map(match => {
      const competitor1Votes = database.getVotes({ matchId: match.id, competitorId: 1 }).length;
      const competitor2Votes = database.getVotes({ matchId: match.id, competitorId: 2 }).length;
